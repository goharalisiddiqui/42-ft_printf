...
/*   Updated: 2021/07/02 17:49:58 by gsiddiqu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libftprintf.h"

static int	ft_printf_conv(char **ptr, va_list *arglst, t_list **strlst)
{
	...
	if (field == NULL)
		return (0);
	ft_lstadd_back(strlst, ft_lstnew(field, **ptr));
	(*ptr)++;
	return (1);
}

static int	ft_printf_nconv(char **ptr, t_list **strlst)
{
	int		nchr;
	char	*str;
	...
	if (str == NULL)
		return (0);
	ft_lstadd_back(strlst, ft_lstnew(str, 'z'));
	while (nchr > 0)
	{
		*str = **ptr;
		str++;
